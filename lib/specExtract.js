...
import { getClassification } from './measurementOntology.js';
import { capabilityOf, gapStatus, routesFor } from './documentCapability.js';

// What each family is expected to specify (drives Present / External / Missing).
// Exported so the registry-consistency test can verify no axis drifts across the maps.
...
const rangeNear = (anchor, text) => {
  ...
  return { min: Math.min(a, b), max: Math.max(a, b), source_text: m[0].replace(/\s+/g, ' ').trim() };
};
 ...
 */
export function extractSpecs(text, fallbackName = '', sourceDocument = '') {
  const t = String(text || '');
  const doc = classify(t);
  const source_document = sourceDocument || fallbackName;
  const nameM = t.match(/שם\s*ה?מוצר[ ,]+([^,\n]+?)\s*(?:,|\n|$)/);
  ...
  for (const p of NUMERIC) {
    ...
    specification[p.axis] = {
      ...
      // Evidence Layer (Fresco): every spec keeps the literal evidence it came from, so the
      // library is auditable and comparable across parser versions — Evidence-first, not
      // just Extraction.
      evidence: {
        source_document, source_section: 'QC block', source_text: r.source_text,
        normalization: { raw: norm.raw, canonical: norm.canonical },
      },
      confidence: norm.normalized ? 1.0 : 0.6,
      classification: getClassification(p.axis),   // Measurement Ontology (Materials Knowledge Layer)
    };
  ...
  for (const q of QUALITATIVE) {
    if (q.aliases.some(a => mentions(a, t))) present_non_standard.push({ axis: q.axis, reason: 'qualitative — no numeric scale yet', classification: getClassification(q.axis) });
  }

  // Resolve each product-expected axis through the Document Capability Map, so a real gap
  // (doc COULD hold it but didn't) is separated from a false one (doc shouldn't hold it →
  // routed to the doc type that should). Excludes already-extracted / declared-empty axes.
  const exp = EXPECTED[doc.family] || {};
  const seen = new Set();
  const missing = [];                  // doc can_contain but absent → REAL gap
  const not_expected = [];             // doc should_not_contain → routed elsewhere (false gap avoided)
  const needs_external_document = [];  // capability silent → undecidable, flag
  for (const ax of [...(exp.external || []), ...(exp.numeric || [])]) {
    if (extracted.has(ax) || declaredEmpty.has(ax) || seen.has(ax)) continue;
    seen.add(ax);
    const entry = { axis: ax, classification: getClassification(ax) };
    const status = gapStatus(doc.document_type, ax);
    if (status === 'Not Expected') not_expected.push({ ...entry, route_to: routesFor(ax) });
    else if (status === 'Missing') missing.push(entry);
    else needs_external_document.push(entry);
  }

  return {
    product, family: doc.family, document_type: doc.document_type,
    document_capability: capabilityOf(doc.document_type),
    specification, present_non_standard, missing, not_expected, needs_external_document,
  };
}
