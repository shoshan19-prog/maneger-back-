...
export function extractSpecs(text, fallbackName = '', sourceDocument = '') {
  ...
  // Each carries its classification so "products with no Mechanical spec" is answerable.
  ...
  for (const ax of [...(exp.external || []), ...(exp.numeric || [])]) {
    ...
    seen.add(ax);
    needs_external_document.push({ axis: ax, classification: getClassification(ax) });
  }
