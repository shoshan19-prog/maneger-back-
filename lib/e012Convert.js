...
// Measured column -> canonical axis (+ its canonical unit/method, and protocol id where the
// axis has a versioned measurement protocol).
export const E012_AXIS_MAP = [
  { col: 'char_density_kg_m3', axis_id: 'char_density', unit: 'kg/m3', method: 'char_mass_over_volume', protocol_id: 'CHAR-DENSITY-PROT-001' },
...
export function e012ToObservations(csvText) {
  ...
  for (const cells of rows) {
    ...
    const protocolVersion = str(g('protocol_version')) || 'v1.0';
    const baseProv = {
    ...
    for (const m of E012_AXIS_MAP) {
      ...
      // Stamp protocol identity on axes that have a versioned protocol (commensurability over time).
      const provenance = m.protocol_id
        ? { ...baseProv, measurement_protocol_id: m.protocol_id, measurement_protocol_version: protocolVersion }
        : baseProv;
      observations.push({
